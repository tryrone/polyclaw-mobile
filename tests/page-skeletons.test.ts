...
describe('page-shaped loading skeletons', () => {
  it('admin games keeps its controls visible while the list loads', () => {
    const source = readFileSync('src/app/(admin)/admin/games.tsx', 'utf8');
    assert.doesNotMatch(source, /if \(initialLoading\) return/);
    assert.match(source, /<AdminGamesListSkeleton \/>/);
  });
