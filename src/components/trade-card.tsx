...
export function TradeCard({ trade }: { trade: Trade }) {
  ...
  return <Pressable onPress={() => router.push(`/trade/${trade.id}`)}><Card style={styles.card}>
    <View style={styles.row}><View style={styles.pills}><StatusPill label={trade.mode} tone={trade.mode === 'PAPER' ? 'neutral' : 'warning'} /><StatusPill label={trade.status} tone={tone(trade.status)} /></View><ArrowUpRight size={18} color={theme.textMuted} /></View>
    ...
    <View style={styles.row}><View><Text style={[styles.label, { color: theme.textMuted }]}>STAKE</Text><Text style={[styles.value, { color: theme.text }]}>{money(trade.plannedStake)}</Text></View><View><Text style={[styles.label, { color: theme.textMuted }]}>EDGE</Text><Text style={[styles.value, { color: theme.lime }]}>{percent(trade.edge)}</Text></View><View><Text style={[styles.label, { color: theme.textMuted }]}>ENTRY</Text><Text style={[styles.value, { color: theme.text }]}>{trade.entryPrice.toFixed(3)}</Text></View></View>
