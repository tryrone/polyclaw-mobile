...
const sources = ['COMBINED', 'BOT', 'MANUAL'] as const;
...
export default function ConsumerPortfolioScreen() {
  ...
  return <Screen refreshControl={<RefreshControl refreshing={resource.loading} onRefresh={resource.refresh} tintColor={theme.accent} />}>
    ...
    {resource.data ? <>
      ...
      <Card><Text style={[styles.copy, { color: theme.textMuted }]}>Bot and manual limits are separate allocations over one reconciled account. Server limits still cap each order and combined daily exposure.</Text><View style={styles.inputs}><BudgetInput label="Bot USDC" value={botBudget ?? String(resource.data.budgets.botBudgetUsdc)} onChangeText={setBotBudget} /><BudgetInput label="Manual USDC" value={manualBudget ?? String(resource.data.budgets.manualBudgetUsdc)} onChangeText={setManualBudget} /></View><ActionButton label="Save budgets" loading={saving} onPress={() => void saveBudgets()} />{message ? <Text style={[styles.inspect, { color: theme.textMuted }]}>{message}</Text> : null}</Card>
      {Platform.OS !== 'web' ? <ActionButton label="Trade a football match" icon={SoccerBall as never} onPress={() => router.push('/football-trade')} /> : null}
