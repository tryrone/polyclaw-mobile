...
import { BookOpen, CheckCircle, Info, Pause, Play, ShieldChevron } from 'phosphor-react-native';
...
const riskOptions = [
  ...
  { value: 'AGGRESSIVE', title: 'Aggressive', detail: '10% per trade · 10% daily · 12% drawdown' },
] as const;
...
export default function BotScreen() {
  ...
  return <Screen refreshControl={<RefreshControl refreshing={resource.loading} onRefresh={resource.refresh} tintColor={theme.accent} />}>
    <Header eyebrow="PAPER QUALIFICATION" title="Your paper bot" action={<StatusPill label={resource.data?.profile.botState ?? 'SYNCING'} tone={resource.data?.profile.botState === 'ACTIVE' ? 'success' : 'warning'} />} />
    <ResourceState loading={resource.loading} error={resource.error} />
    {resource.data ? <>
      {!resource.data.access.active ? <Card variant="raised"><View style={styles.noticeTitle}><Info size={21} color={theme.warning} weight="fill" /><Text style={[styles.heading, { color: theme.text }]}>{pilotAccessMode ? 'Pilot access is inactive' : 'Subscription access is inactive'}</Text></View><Text style={[styles.copy, { color: theme.textMuted }]}>{!pilotAccessMode ? 'Open Account to restore or manage the subscription required for new bot positions.' : resource.data.access.pilotRequest?.status === 'PENDING' ? 'Your request is waiting for a pilot administrator. You can still review your portfolio, but the paper bot cannot create new positions until access is renewed.' : resource.data.access.pilotGrant?.status === 'EXPIRED' ? `Your pilot grant expired${resource.data.access.pilotGrant.expiresAt ? ` on ${new Date(resource.data.access.pilotGrant.expiresAt).toLocaleDateString()}` : ''}. Request renewal to continue building your paper history.` : 'A named internal pilot account needs a 30-day grant before the paper bot can run.'}</Text><ActionButton label={!pilotAccessMode ? 'Open account' : resource.data.access.pilotRequest?.status === 'PENDING' ? 'Access request pending' : 'Request pilot access'} variant="secondary" loading={busy} disabled={busy || (pilotAccessMode && resource.data.access.pilotRequest?.status === 'PENDING')} onPress={() => pilotAccessMode ? void act('request-access') : router.push('/account')} /></Card> : null}
      <ActionButton label="Open paper-to-live setup guide" icon={BookOpen as never} variant="secondary" onPress={() => router.push('/getting-started')} />
