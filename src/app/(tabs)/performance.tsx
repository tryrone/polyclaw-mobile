...
import { Card, Header, Metric, money, percent, ResourceState, Screen, StatusPill } from '@/components/ui-kit';
...
export default function PerformanceScreen() {
  ...
  return <Screen refreshControl={<RefreshControl refreshing={resource.loading} onRefresh={resource.refresh} tintColor={theme.accent} />}><Header eyebrow="SETTLED RESULTS" title="Performance" /><ResourceState loading={resource.loading} error={resource.error} stale={resource.stale} />
    {data ? <><Staggered index={0}><Card><View style={styles.metrics}><Metric label="Net P&L" value={money(data.netPnl)} numeric={data.netPnl} format={money} accent={data.netPnl >= 0} /><Metric label="Hit rate" value={percent(data.hitRate)} numeric={data.hitRate} format={percent} /><Metric label="Wins" value={String(data.wins)} /><Metric label="Losses" value={String(data.losses)} /></View><PerformanceChart series={data.series} /></Card></Staggered>
      <Staggered index={1}><Card><View style={styles.row}><View style={{ flex: 1 }}><Text style={[styles.heading, { color: theme.text }]}>Paper-to-live gate</Text><Text style={[styles.copy, { color: theme.textMuted }]}>{data.paperGate.settled}/{data.paperGate.requiredSettled} trades · {data.paperGate.days}/{data.paperGate.requiredDays} days · positive P&L required</Text></View><StatusPill label={data.paperGate.eligible ? 'ELIGIBLE' : 'LOCKED'} tone={data.paperGate.eligible ? 'success' : 'warning'} /></View></Card></Staggered>
    </> : null}
  </Screen>;
}
